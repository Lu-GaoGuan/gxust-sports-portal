...
- 第一阶段：六个独立路由的迎新展示页面、内容 API、Django Admin 内容管理和审核制留言已实现。当前前端按阶段要求使用模拟数据，尚未接入 API。
...
- `GET /api/messages/`：已审核通过的留言。
- `POST /api/messages/`：提交待审核留言，字段为 `nickname` 和 `content`。
