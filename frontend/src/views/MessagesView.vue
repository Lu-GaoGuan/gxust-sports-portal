...
const canSubmit = computed(() => form.value.nickname.trim().length > 0 && form.value.content.trim().length >= 2 && reviewAcknowledged.value && !submitting.value)
...
async function submit() {
  ...
  try {
    ...
    successMessage.value = `留言已提交（编号 ${result.id}），当前处于待审核状态，审核通过后才会公开显示。`
  } catch (error) {
...
<template>
  <main><PageBanner eyebrow="MESSAGE BOARD" title="留言交流" description="有问题、有想法，或者只是想说声你好，都欢迎留在这里。" number="06" />
    <section class="content-section message-layout"><form @submit.prevent="submit"><p class="section-kicker">LEAVE A MESSAGE</p><h2>给体育部留句话</h2><label class="interactive-field"><span class="field-heading">你的昵称<small>{{ form.nickname.length }} / 50</small></span><input v-model="form.nickname" required maxlength="50" placeholder="怎么称呼你" @input="resetSubmitNotice"></label><label class="interactive-field"><span class="field-heading">留言内容<small>{{ form.content.length }} / 1000</small></span><textarea v-model="form.content" required minlength="2" maxlength="1000" rows="6" placeholder="想问的问题或想说的话" @input="resetSubmitNotice"></textarea><i class="content-meter" aria-hidden="true"><span :style="{ width: `${contentProgress}%` }"></span></i></label><label class="review-checkbox"><input v-model="reviewAcknowledged" type="checkbox"><span class="review-checkmark" aria-hidden="true"></span><span>我已了解：留言提交后需经管理员审核，通过后才会公开展示。</span></label><p class="form-hint">请勿提交联系方式、身份证号等个人敏感信息。</p><button class="button button-blue" type="submit" :disabled="!canSubmit">{{ submitting ? '正在提交…' : '提交留言' }}</button><p v-if="successMessage" class="success-message" role="status">{{ successMessage }}</p><p v-if="submitError" class="submit-error" role="alert">{{ submitError }}</p></form>
      <div class="message-list-area"><div v-if="loading" class="data-state"><span class="state-spinner"></span><p>正在加载审核通过的留言…</p></div><div v-else-if="listError" class="data-state error"><p>{{ listError }}</p><button type="button" @click="loadMessages">重新加载</button></div><div v-else-if="!messages.length" class="data-state empty"><h3>暂无公开留言</h3><p>还没有审核通过的留言，欢迎提交第一条。</p></div><TransitionGroup v-else name="message-list" tag="div" class="message-wall"><article v-for="message in messages" :key="message.id"><div class="avatar">{{ message.nickname.slice(0, 1) }}</div><div><header><b>{{ message.nickname }}</b><small>{{ formatTime(message.submitted_at) }}</small></header><p>{{ message.content }}</p></div></article></TransitionGroup></div>
    </section>
  </main>
</template>
